...
10M급 실험에서는 checkpoint와 batch도 큰 prefix에 맞게 조정한다. 기본값은 `CQPCFG_CHECKPOINT_INTERVAL_RECORDS=10000`, `CQPCFG_BATCH_SIZE=65536`, `CQPCFG_MAX_BATCH_PAYLOAD_BYTES=1048576`, `CQPCFG_MAX_CHUNK_SIZE=8192`, `CQPCFG_LEASE_STRATEGY=rank_window_probability_mass`, `CQPCFG_TARGET_CHUNK_PROBABILITY_MASS=0.01`, `CQPCFG_RANK_WINDOW_SIZE=65536`이다.
...
## Nginx stream proxy로 외부 worker 붙이기

SSH tunnel 없이 외부 worker를 붙일 때는 Nginx의 `stream` 모듈로 CQPCFG의 raw TCP subchannel을 그대로 프록시한다. HTTP reverse proxy가 아니므로 `location`이나 path 기반 라우팅을 쓰지 않는다.

기본 매핑은 다음과 같다.

| 공개 포트 | 내부 tracker 포트 | 역할 |
|---:|---:|---|
| `15555` | `5555` | control |
| `15556` | `5556` | candidate batch |
| `15557` | `5557` | role/bootstrap |
| `15558` | `5558` | batch ACK |
| `15559` | `5559` | model page/block |

서버에서 한 번만 설정한다.

```bash
CQPCFG_NGINX_PUBLIC_BASE_PORT=15555 \
CQPCFG_NGINX_UPSTREAM_BASE_PORT=5555 \
CQPCFG_NGINX_ALLOW_CIDR=<worker-public-ip> \
./experiments/docker/nginx/install_cqpcfg_stream.sh
```

첫 공개 포트가 이미 사용 중이거나 외부 방화벽에서 막혀 있으면 base port만 바꾼다. 예를 들어 `15556`을 base로 쓰면 공개 포트는 `15556~15560`이 되고, worker도 같은 base port로 접속해야 한다.

```bash
CQPCFG_NGINX_PUBLIC_BASE_PORT=15556 \
CQPCFG_NGINX_UPSTREAM_BASE_PORT=5555 \
CQPCFG_NGINX_ALLOW_CIDR=<worker-public-ip> \
./experiments/docker/nginx/install_cqpcfg_stream.sh
```

SSH 같은 비대화형 환경에서 `sudo` 비밀번호를 stdin으로 넘겨야 하면 다음처럼 실행한다.

```bash
printf '%s\n' '<sudo-password>' | \
  SUDO='sudo -S' CQPCFG_NGINX_PUBLIC_BASE_PORT=15556 \
  ./experiments/docker/nginx/install_cqpcfg_stream.sh
```

tracker는 내부 loopback에 bind하고, worker에게는 Nginx의 공개 주소를 advertise한다.

```bash
CQPCFG_MODEL_PATH=../CQDAGPCFG/examples/artifacts/rockyou_train/model.json \
CQPCFG_JOB_SPEC_PATH=/tmp/cqdagpcfg-split/job-spec.json \
CQPCFG_BIND=cqpcfg://127.0.0.1:5555 \
CQPCFG_PUBLIC_CONTROL_CONNECT=cqpcfg://tracker.example.com:15555 \
CQPCFG_PUBLIC_BATCH_CONNECT=cqpcfg://tracker.example.com:15556 \
CQPCFG_PUBLIC_ACK_CONNECT=cqpcfg://tracker.example.com:15558 \
CQPCFG_PUBLIC_MODEL_CONNECT=cqpcfg://tracker.example.com:15559 \
python experiments/cqpcfg_experiment.py tracker
```

worker는 공개 base endpoint 하나만 알면 된다. 내부적으로 `15555~15559` subchannel로 확장된다.
...
위 예시에서 Nginx base port를 `15556`으로 바꿨다면 worker도 `CQPCFG_PORT=15556`으로 맞춘다.
